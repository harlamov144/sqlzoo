...
-- 8. Exclusive OR (XOR). Show the countries that are big by area (more than 3 million) or big by population (more
...
-- Use the ROUND function to show the values to two decimal places. For South America show population in millions and GDP
...
-- that is 12 zeros). Round this value to the nearest 1000. Show per-capita GDP for the trillion dollar countries to the
-- nearest $1000.
...
-- where the name and the capital have the same number of characters. You can use the LENGTH function to find the number
...
-- first letters of each match. Don't include countries where the name and the capital are the same word. You can use the
-- function LEFT to isolate the first character. You can use <> as the NOT EQUALS operator.
SELECT name, capital
...
-- they have more than one word in the name. Find the country that has all the vowels and no spaces in its name. You can
-- use the phrase name NOT LIKE '%a%' to exclude characters from your results. The query shown misses countries like Bahamas
-- and Belarus because they contain at least one 'a'
SELECT name
FROM world
